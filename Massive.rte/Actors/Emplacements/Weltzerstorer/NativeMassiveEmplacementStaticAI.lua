...
require("Actors/Emplacements/Weltzerstorer/MassiveEmplacementBehaviors")
require("Actors/Emplacements/Weltzerstorer/MassiveEmplacementStaticBehaviors")

NativeMassiveEmplacementStaticAI = {}

function NativeMassiveEmplacementStaticAI:Create(Owner)
	local Members = {}
...
function NativeMassiveEmplacementStaticAI:Update(Owner)
...
function NativeMassiveEmplacementStaticAI:Destroy(Owner)
	-- Tell the coroutines to abort to avoid memory leaks
	if self.Behavior then
		local msg, done = coroutine.resume(self.Behavior, self, Owner, true)
	end
...
function NativeMassiveEmplacementStaticAI:CreatePatrolBehavior(Owner)
	self.NextBehavior = coroutine.create(TurretBehaviors.Patrol)
	self.NextCleanup = nil
	self.NextBehaviorName = "Patrol"
end

function NativeMassiveEmplacementStaticAI:CreateSentryBehavior(Owner)
...
function NativeMassiveEmplacementStaticAI:CreateAttackBehavior(Owner)
...
function NativeMassiveEmplacementStaticAI:CreateSuppressBehavior(Owner)
	if Owner.FirearmIsReady then
...
function NativeMassiveEmplacementStaticAI:CreateFaceAlarmBehavior(Owner)
	self.NextBehavior = coroutine.create(HumanBehaviors.FaceAlarm)
	self.NextBehaviorName = "FaceAlarm"
	self.NextCleanup = nil
end

function NativeMassiveEmplacementStaticAI:CreatePinBehavior(Owner)
