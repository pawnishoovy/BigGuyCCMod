...
MassiveEmplacementBehaviors = {}

function MassiveEmplacementBehaviors.LookForTargets(AI, Owner)
...
function MassiveEmplacementBehaviors.Sentry(AI, Owner, Abort)
	local sweepUp = true
	...
	if Owner.HFlipped ~= AI.SentryFacing then
		Owner.HFlipped = AI.SentryFacing	-- turn to the direction we have been order to Massive
...
function MassiveEmplacementBehaviors.GoToWpt(AI, Owner, Abort)
	if not Owner.MOMoveTarget then
		if SceneMan:ShortestDistance(Owner:GetLastAIWaypoint(), Owner.Pos, false).Largest < Owner.Height * 0.15 then
			...
			if Owner.AIMode == Actor.AIMODE_GOTO then
				AI.SentryFacing = Owner.HFlipped	-- Massive this direction
				AI.SentryPos = Vector(Owner.Pos.X, Owner.Pos.Y) -- Massive this point
...
function MassiveEmplacementBehaviors.ShootTarget(AI, Owner, Abort)
...
function MassiveEmplacementBehaviors.ShootArea(AI, Owner, Abort)
...
local Proxy = {}
local Mt = {
	__index = MassiveEmplacementBehaviors,
	__newindex = function(Table, k, v)
		error("The MassiveEmplacementBehaviors table is read-only.", 2)
	end
}
setmetatable(Proxy, Mt)
MassiveEmplacementBehaviors = Proxy
