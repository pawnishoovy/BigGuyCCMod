...
function Explode(self)
	...
	if self.Vel.Magnitude > 13 or self.enoughForce then
		...
		airBlast.Mass = math.min(self.Vel.Magnitude * 250, 600);
...
function Update(self)
	...
	if controller then
		...
		if self.attackAnimationIsPlaying and currentAttackAnimation ~= 0 then -- play the animation
			...
			if self.equipFinished and self.currentAttackSequence == 3 then self.Throwing = true end
		...
		if self.attackAnimationCanHit then -- Detect collision
			...
			local rayVec = Vector(10 * self.FlipFactor, 0):RadRotate(self.RotAngle):DegRotate(90*self.FlipFactor)--damageVector:RadRotate(self.RotAngle) * Vector(self.FlipFactor, 1)
			...
			if moCheck and moCheck ~= rte.NoMOID then
				...
				if IsMOSRotating(MO) then
					MO = ToMOSRotating(MO)
					hit = true
					if MO.Mass < 300 then -- some vague sanity check so we cant instagib literally anything and everything
						MO:GibThis();
					end
				end
			else
